    ...
    <div className={`markdown-content text-zinc-300 leading-relaxed font-sans text-xs ${className}`}>
      <ReactMarkdown
        components={{
          ...
          h2: ({ children }) => (
            <h2 className="text-xs font-bold text-white uppercase tracking-wider mt-2.5 mb-1 text-[#06b6d4]">
              {children}
            </h2>
          ),
          h3: ({ children }) => (
          ...
          ul: ({ children }) => <ul className="space-y-1 my-1.5 pl-4 list-disc marker:text-[#06b6d4]">{children}</ul>,
          ...
          code: ({ children }) => (
            <code className="px-1.5 py-0.5 rounded bg-zinc-800 text-[#06b6d4] font-mono text-[11px] border border-white/10">
              {children}
            </code>
          ),
          blockquote: ({ children }) => (
            <blockquote className="border-l-2 border-[#06b6d4] pl-3 my-2 text-zinc-400 italic">
              {children}
            </blockquote>
          ),
        }}
